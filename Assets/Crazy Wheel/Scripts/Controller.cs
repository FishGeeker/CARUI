...
public class Controller : MonoBehaviour {
	...
	void Update()
	{
		if(Input.GetKeyDown(KeyCode.Escape) && menu==false && DialController.GameOver)
			Home();
	...
	public void StartGame(int difficulty)
	{
		...
		if(difficulty==0)
		{
			DialController.ArraySize=3;
			Diff=Level.Easy;
		}
		if(difficulty==1)
		{
			DialController.ArraySize=4;
			Diff=Level.Medium;
		}
		if(difficulty==2)
		{
			DialController.ArraySize=6;
			Diff=Level.Hard;
		}
		...
		//Show dial
		DialController.ShowDial();

		//Reset the dial's initial speed
		if(Diff==Level.Easy)
			DialController.InitialSpeed=EasyDialSpeed;
		else if(Diff==Level.Medium)
			DialController.InitialSpeed=MediumDialSpeed;
		else if(Diff==Level.Hard)
			DialController.InitialSpeed=HardDialSpeed;

		//Change dial color
		DialController.ChangeDialColor();
	...
	public void GameOver()
	{
		...
		//Hide the dial
		DialController.HideDial();

		//Set the GameOver bool to true
		DialController.GameOver=true;
